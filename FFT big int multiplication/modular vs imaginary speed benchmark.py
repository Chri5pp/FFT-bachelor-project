...
def benchmark():
    ...
    for n in sizes:
        print(f"Timing n being about 2^{n.bit_length()}", end="")
        ...
        for _ in range(samples): 
            ...
            mod_time += time_algorithm(multiply_big_positive_integers, a, b)
            imaginary_time += time_algorithm(multiply_big_positive_integers_2, a, b)

        imaginary_times.append(imaginary_time / samples)
        mod_times.append(mod_time / samples)
        print(f": Done")

    plt.figure()
    plt.plot(sizes, mod_times)
    plt.plot(sizes, imaginary_times)

    plt.xlabel("Size of random number (0 to n)")
    plt.ylabel("Time (seconds)")
    plt.xscale("log")
    plt.yscale("log")
    plt.legend(["Modular FFT", "Imaginary FFT"])
    plt.title("Integer multiplication speed comparison")
    plt.show()
